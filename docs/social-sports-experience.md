...
- **Perfil público de publicador** (`/publicadores/[id]`): el paso natural siguiente. El backend ya expone `GET /api/perfiles-publicadores` (consumido ahora en la home) y `GET /api/perfiles-publicadores/{id}/imagenes`; falta un endpoint de detalle individual por id/slug, slug de publicador para URL amigable y contador público de seguidores. Mientras tanto la vista es `/explorar?perfilPublicadorId=N` (ahora estable).
- **Deuda de contrato para la visión social** (documentada, no implementable solo con frontend): favoritos/preferencias por cuenta (hoy `localStorage` por dispositivo, prometido en la UI), likes con contadores, `fechaPublicacion` en `ActividadDTO` (sin ella no hay "publicado hace X"), paginación del feed (top 20 fijo), `imagenPrincipalUrl` en el DTO público de listados (las cards públicas siempre caen al placeholder por deporte), refresh token (la sesión muere por pestaña).
