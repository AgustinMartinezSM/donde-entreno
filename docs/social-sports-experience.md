...
- **La galería aprobada no se muestra en ningún lado público** (detectado en el QA del bloque 4, 2026-08-08). El contrato alcanza: `GET /api/actividades/{slug}/imagenes` existe, es público y ya filtra `APROBADA` + activa — verificado en producción con la actividad `karate`, que devuelve su imagen `GALERIA` servida desde el bucket público de Supabase. Lo que falta es UI: `/actividades/[slug]` nunca renderiza la galería. **Es sólo frontend**, no hace falta tocar backend.
- **El detalle público ignora la imagen PRINCIPAL aprobada** (mismo QA). `/actividades/[slug]` resuelve su imagen con `actividad.imagenes?.find(t === "PRINCIPAL")`, pero el `ActividadDTO` público **no tiene** campo `imagenes` (verificado contra producción: el detalle de `karate` no lo trae). O sea que `imagenPrincipal` es siempre `undefined` y el hero y el `og:image` caen siempre a la ilustración por deporte, aun con una PRINCIPAL aprobada. El dato correcto ya viaja en el DTO desde `5da9d4f`: es `actividad.imagenPrincipalUrl`, el mismo que ya usan las cards. **Es sólo frontend**: cambiar la fuente en las dos lecturas (`generateMetadata` y el render) y borrar el `find` muerto.
