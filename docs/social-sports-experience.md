...
- **Perfil público de publicador — deuda de backend para la V2**: ~~resuelve el perfil filtrando el listado público~~ → **hecho** (`f7e276c` + `c81e05d`): existe `GET /api/perfiles-publicadores/{id}`, aditivo, con el mismo DTO y el mismo criterio de visibilidad que el listado (`activo = true`, para que un perfil no pueda estar en la lista y dar 404 en su detalle), y `obtenerPerfilPublicadorPorId` ya lo consume. Medido en producción: 0,22 KB contra 2,32 KB, y la página lo pide dos veces por vista (`generateMetadata` + render, ambos `no-store`). ~~Contador público de seguidores~~ → **hecho** (`915ddac` backend + `92db20d` frontend): campo aditivo `cantidadSeguidores`, sin migración (la tabla `seguimiento_publicador` existe desde el script 17). El detalle usa el `count` directo; el listado usa un query agrupado nuevo para no disparar N+1, y los perfiles sin seguidores se completan con cero porque no vuelven en el `GROUP BY`. El chip aparece recién a partir del primer seguidor: con todos los perfiles en cero, un "0 seguidores" repetido los hace ver abandonados. Sigue pendiente el **slug amigable** para la URL, que es aditivo de backend pero necesita migración.
- Nota de deploy para bloques que agregan endpoints: el backend va en un push aparte y se verifica en producción **antes** de apuntar el frontend, porque el cliente no puede distinguir "el recurso no existe" de "el endpoint todavía no existe" — los dos son 404. Se distinguen por el cuerpo: el handler propio trae su mensaje, el genérico dice `"Recurso no encontrado."`.
- **Deuda de contrato para la visión social** (documentada, no implementable solo con frontend): favoritos/preferencias por cuenta (hoy `localStorage` por dispositivo, prometido en la UI), likes con contadores, paginación del feed (top 20 fijo), refresh token (la sesión muere por pestaña). ~~`imagenPrincipalUrl` en el DTO público~~ → **hecho** (rama `feature/imagen-principal-dto`): campo aditivo asignado por `ImagenService.asignarImagenPrincipal` en listados, detalle y feed — un query batch por página (sin N+1), respetando la política de moderación (solo `APROBADA` + activa + tipo `PRINCIPAL`). El frontend ya lo consumía con fallback, sin cambios de cliente. **Hallazgo del QA**: las URLs `/uploads/...` del seed siempre fueron aspiracionales — no hay archivos en el repo, no hay ResourceHandler y `/uploads/**` responde 401 (preexistente: afectaba igual al detalle y al og:image). Servirlas desde disco local está vetado (Render efímero, regla 6). El circuito completo se enciende con el **bloque 4** (Supabase Storage): `construirUrlImagenBackend` pasa URLs absolutas intactas, así que con URLs de Supabase las cards funcionan solas — solo habrá que sumar el host de Supabase a `images.remotePatterns` de `next.config`. Mientras tanto, las cards intentan la imagen y caen al fallback por deporte sin rotura visible (verificado en navegador). ~~`fechaPublicacion` en `ActividadDTO`~~ → **hecho** (rama `feature/fecha-publicacion-dto`): campo aditivo mapeado de `createdAt` (las actividades nacen al aprobarse, así que es su fecha de publicación real; sin migración), con "Publicada hace X" (`Intl.RelativeTimeFormat` es-AR) en las cards del feed y el detalle. Campo opcional en el frontend: tolera cualquier orden de llegada de los deploys.
- ~~**La galería aprobada no se muestra en ningún lado público**~~ (detectado en el QA del bloque 4, 2026-08-08) → **hecho**. El contrato ya alcanzaba: `GET /api/actividades/{slug}/detalle` devuelve `actividad + horarios + imagenes` y el backend filtra `APROBADA` + activa; el service las normalizaba en `ActividadDetalle.imagenes`. Lo que faltaba era UI: el detalle solo leía la `PRINCIPAL` para el hero y descartaba el resto, así que las fotos de galería que el publicador subía y el admin aprobaba no aparecían en ninguna superficie. Ahora `/actividades/[slug]` muestra una sección "Fotos de la actividad" con las imágenes restantes. Sin cambios de backend.
- Nota de QA sobre lo anterior: durante el diagnóstico se llegó a afirmar que el detalle ignoraba la `PRINCIPAL` aprobada. **Es falso**, y quedó registrado para que no se repita el error: la confusión vino de probar `GET /api/actividades/{slug}` (el DTO de listado, que efectivamente no trae `imagenes`) en vez de `GET /api/actividades/{slug}/detalle`, que es el que consume la página. El hero y el `og:image` sí usan la `PRINCIPAL` aprobada cuando existe.
